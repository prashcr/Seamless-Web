...
Music({
    ...
    created() {
      let urls = {
        hiphop: "https://soundcloud.com/alltrapnation",
      ...
      SC.resolve(urls.hiphop).then(data => {
        return data.id
      }).then(id => {
        ...
        tracks.forEach(track => {
          console.log(track);
          self.tracks().push({id: track.id, title: track.title, artwork: track.artwork_url, stream: track.stream_url + "?client_id=2b7b69947b17fa8e085f79616c35dafc"})
        })
      })
    },
    destroyed() {
